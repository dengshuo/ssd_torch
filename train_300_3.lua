...
function train_300_3(trainTarget, trainName)   
  --tot_error = 0
  --tot_cls_err = 0
  --tot_loc_err = 0
  --cnt_error = 0
  print("training 300x300_3 model ")    
  model:training()
  -- shuffle imgs
  ...
  for t = 1,trainSz,batchSz do
    ...
    if tot_iter % 1000 == 0 then
      ...
      local filename = paths.concat(model_dir, '300x300_3_' .. tot_iter .. 'model.net')
      os.execute('mkdir -p ' .. sys.dirname(filename))
      print('==> saving model to '..filename)
      model:clearState()
      torch.save(filename, model)
    
      fp_err = io.open(resultDir .. "_300x300_3_loss.txt","a")
