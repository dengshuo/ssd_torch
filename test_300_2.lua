...
function test_300x300_2(testTarget, testName)
    ...
    for lid = 1,classNum-1 do
      ...
      if type(resultBB[lid]) == "userdata" then
        for rid = 1,resultBB[lid]:size()[1] do                    
          ...
          local score = resultBB[lid][rid][5]
          ...
          --input1 = torch.reshape(input,inputDim,imgSz,imgSz)
          --input2 = drawRectangle(input1,xmin,ymin,xmax,ymax)
          --draw BB
          --file_path ='./test_result/'
          --image.save(file_path .. tostring(t) .. "_" .. classList[lid] .. ".jpg", input2)                                    
        end
      end
      fp_result:close()
    end --lid
